...
const ACTION_ICONS := {
	...
	&"shield_strike": "res://assets/art/UI/sword-and-shield-icon.png",
...
func refresh(unit: SimUnit) -> void:
	...
	# Shield-up state borrows the action icon slot; otherwise show the action's
	# icon, falling back to the text label for actions without art yet.
	var icon_path: String = ACTION_ICONS.get(unit.creature.action, "")
	if unit.shield_active:
		icon_path = "res://assets/art/UI/shield-icon.png"
